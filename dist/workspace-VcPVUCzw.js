import { d as resolveRequiredHomeDir } from "./paths-DPhYhCig.js";
import { $ as shouldLogVerbose, I as pathExists$1, J as danger, Y as info, Z as logVerboseConsole, ot as getLogger, r as defaultRuntime, t as createSubsystemLogger, tt as warn, z as resolveUserPath } from "./subsystem-A5aAh1zO.js";
...
async function resolveBoundaryPath(params) {
	...
	const rootCanonicalPath = params.rootCanonicalPath ? path.resolve(params.rootCanonicalPath) : await resolvePathViaExistingAncestor(rootPath);
	...
	const canonicalOutsideLexicalPath = resolveCanonicalOutsideLexicalPath({
		absolutePath,
		outsideLexicalCanonicalPath: lexicalInside ? void 0 : await resolvePathViaExistingAncestor(absolutePath)
	});
	...
	if (!lexicalInside) {
		...
		return buildResolvedBoundaryPath({
			...
			rootPath,
			rootCanonicalPath,
			kind: await getPathKind(absolutePath, false)
		});
	}
...
function resolveBoundaryPathSync(params) {
	const rootPath = path.resolve(params.rootPath);
	const absolutePath = path.resolve(params.absolutePath);
	const rootCanonicalPath = params.rootCanonicalPath ? path.resolve(params.rootCanonicalPath) : resolvePathViaExistingAncestorSync(rootPath);
	const lexicalInside = isPathInside(rootPath, absolutePath);
	const canonicalOutsideLexicalPath = resolveCanonicalOutsideLexicalPath({
		absolutePath,
		outsideLexicalCanonicalPath: lexicalInside ? void 0 : resolvePathViaExistingAncestorSync(absolutePath)
	});
	assertLexicalBoundaryOrCanonicalAlias({
		skipLexicalRootCheck: params.skipLexicalRootCheck,
		lexicalInside,
	...
	if (!lexicalInside) {
		...
		return buildResolvedBoundaryPath({
			...
			rootPath,
			rootCanonicalPath,
			kind: getPathKindSync(absolutePath, false)
		});
	}
...
async function readLexicalStatAsync(params) {
	try {
		return await fs.lstat(params.state.lexicalCursor);
	} catch (error) {
...
function readLexicalStatSync(params) {
	try {
		return fs$1.lstatSync(params.state.lexicalCursor);
	} catch (error) {
		if (handleLexicalLstatFailure({
			error,
			state: params.state,
...
async function resolveBoundaryPathLexicalAsync(params) {
	...
	for (const { idx, segment, isLast } of iterateLexicalTraversal(state)) {
		const stat = await readLexicalStatAsync({
			...sharedStepParams,
...
function resolveBoundaryPathLexicalSync(params) {
	...
	for (const { idx, segment, isLast } of iterateLexicalTraversal(state)) {
		const stat = readLexicalStatSync({
			...sharedStepParams,
			missingFromIndex: idx
		});
		if (!stat) break;
